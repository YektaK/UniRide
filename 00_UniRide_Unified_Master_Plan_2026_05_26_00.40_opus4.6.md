...
| 2 | Holistic solver constraint-validation pass | 3.3 / 4.2 | Depends on real dataset import and benchmark rows | First real CVRPLIB/Solomon slice persisted without runtime errors, but OR-Tools/PyVRP/VROOM rows produced suspiciously low costs and one-vehicle CVRPTW solutions on Solomon cases. Treat current holistic rows as adapter smoke evidence only until capacity/time-window constraint mapping is audited against BKS route files. |
...
Recommended next task: audit OR-Tools/PyVRP/VROOM core adapter constraint mapping against the tracked CVRPLIB/Solomon BKS route files, then rerun `python -m academic_benchmark.run_matrix_benchmark --problem-types cvrp cvrptw ...` for broader real evidence.
