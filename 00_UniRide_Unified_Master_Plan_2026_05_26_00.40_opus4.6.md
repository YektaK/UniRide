...
Recommended next task: add an all-imported-problem validation profile or continue production strategy migration only where `optimizer_api/strategies/*` still owns critical algorithm behavior; current OR-Tools/PyVRP academic benchmark path is stable enough for repeatable local testing.
