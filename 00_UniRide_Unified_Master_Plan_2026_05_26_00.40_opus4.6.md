...
| 6 | Production strategy migration: holistic/split wrapper thinning | 4.x | Depends on holistic comparison clarity | COVERED for current active wrappers. `optimizer_api` remains responsible for API mapping; OR-Tools/PyVRP/VROOM solver logic and GWO/HHO split search logic are guarded as core-owned. Continue only if future audits find active algorithm-critical logic in `optimizer_api/strategies/*`. |
...
| 8 | Full regression gate before release branch | Cross-phase | Depends on chosen release scope | CURRENTLY GREEN for the main local gate: `academic_benchmark\tests` passes with 190 tests, `uniride_core\tests` passes with 202 tests, focused optimizer API compatibility/migration tests pass with 31 tests, and `npm run typecheck` passes. Remaining release-scope work is broader optimizer API/frontend runtime coverage if requested. |
...
- Task 49: Repeatable matrix-validation profiles added for academic benchmark readiness. `academic_benchmark.matrix_validation_plan` now supports `--profile quick-real` for the existing curated real-instance validation and `--profile all-imported` for all stored CVRP/CVRPTW rows in SQLite. The all-imported profile defaults to one `time10-scale1000` variant, OR-Tools + PyVRP, `n_runs=1`, and `limit=100000`, matching the recent all-imported validation shape while still allowing explicit CLI overrides for problems, problem types, runs, limit, and variants. Verification: `python -m pytest academic_benchmark\tests\test_matrix_validation_plan.py -q` passes with 5 tests; broader `python -m pytest academic_benchmark\tests -q` passes with 190 tests; live profile run `python -m academic_benchmark.matrix_validation_plan --profile all-imported --run-id-prefix matrix-validation-all-imported-20260604` completed as `matrix-validation-all-imported-20260604-time10-scale1000` with 142 saved rows, `errors=[]`, 142 feasible rows, 0 failed rows, 0 capacity violation rows, 0 time-window violation rows, 75 positive vehicle-gap rows, 75 suppressed distance-gap rows, and 0 unexpected distance gaps when vehicle count is worse than BKS.
