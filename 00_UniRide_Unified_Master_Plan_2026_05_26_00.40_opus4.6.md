...
Recommended next task: run multi-seed real CVRPLIB/Solomon validation with solver time-limit variants; treat pyvroom's Windows matrix buffer compatibility as a separate optional task because VROOM CVRPTW rows are already excluded safely when native execution fails.
