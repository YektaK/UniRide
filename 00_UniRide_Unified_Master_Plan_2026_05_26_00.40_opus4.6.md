...
| Split engines (GA/PSO/GWO/HHO Split) | planned via direct delegation | planned via direct delegation | ✅ | ✅ | ✅ | Split strategy internals moved to `uniride_core`; wrappers remain in `optimizer_api`. For pure TSP/ATSP, split engines should call the corresponding direct core engine and return one tour/route rather than forcing artificial vehicle splitting. |
| Holistic CVRP adapters (OR-Tools, PyVRP, VROOM) | planned per adapter | planned per adapter | ✅ | partial | partial | Core-owned adapters exist. Optional dependency coverage verified locally for OR-Tools, PyVRP, and VROOM's `vroom` module. Next step is unified `solve_problem(RoutingProblem)` dispatch with explicit unsupported-case reporting. |
...
| 2 | Split-engine TSP/ATSP compatibility dispatch | Phase 2 / 3.3 | Independent; should be done before claiming all algorithm families are benchmarkable on TSP/ATSP | Add tests proving `GA/PSO/GWO/HHO Split` accept TSP and ATSP `RoutingProblem`s. Implement by delegating to direct core TSP/ATSP engines and returning a single tour/route. Keep real split decoding for CVRP/CVRPTW/UniRide only. |
| 3 | Holistic adapter unified dispatch | 3.3 / 4.2 | Depends on optional solver availability; partly independent from dataset import | Add `solve_problem(RoutingProblem)` coverage for OR-Tools, VROOM, and PyVRP. OR-Tools/VROOM should cover TSP/CVRP/CVRPTW where supported. PyVRP TSP can be modeled as one vehicle if library behavior validates; ATSP must be explicitly verified or reported unsupported. |
| 4 | Holistic solver comparison pass | 3.3 / 4.2 | Depends on tasks 1 and 3 | Run OR-Tools, PyVRP, and VROOM on comparable CVRP/CVRPTW instances; record infeasible/unsupported cases explicitly rather than hiding them. |
| 5 | Additional controlled FCM-SRS validation | Phase 6 candidate | Independent after current controlled runs | DONE for first controlled pass on 144-node GA/PSO, 256-node PSO, and 500-node PSO. Continue only if targeting FCM promotion; add more seeds, dimensions, and real TSPLIB-style instances before promoting. |
| 6 | Promotion-quality config generation | 4.1 / 4.2 | Depends on tasks 1 and 4 | Existing promotion manager works; rerun after real benchmark data is populated so generated configs are based on meaningful CVRP/CVRPTW evidence. |
| 7 | Production strategy migration: remaining SOTA wrappers | 4.x | Independent of academic runs, lower priority | E2BSO/R2DMA/P-AOEA wrappers delegate to core TSP solvers but still use typed constructor configs. Decide whether per-request SOTA config overrides are needed before changing behavior. |
| 8 | Production strategy migration: holistic wrapper thinning | 4.x | Depends on holistic comparison clarity | Keep `optimizer_api` responsible for API mapping only; move any remaining algorithm-critical OR-Tools/PyVRP/VROOM logic into `uniride_core` if found during comparison runs. |
| 9 | Packaging and requirements cleanup | Cross-phase | After dependency decisions | Pin or document optional packages (`vrplib`, `ortools`, `pyvrp`, `pyvroom`) consistently for local dev, CI, and production. Keep graceful fallback imports. |
| 10 | Full regression gate before release branch | Cross-phase | Depends on chosen release scope | Run academic/core pytest, focused optimizer API tests, and frontend typecheck together; document known optional skips/failures separately. |
