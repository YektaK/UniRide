...
Recommended next task: decide whether to add a small approved CLI profile feature for repeatable all-imported validation, or continue production strategy migration only where `optimizer_api/strategies/*` still owns critical algorithm behavior; current OR-Tools/PyVRP academic benchmark path is stable enough for repeatable local testing.
