...
|:------|:-----|:------|:-----------|:------|
| 1 | Broaden CVRPLIB/Solomon benchmark execution | 3.2 / 3.3 | Depends on verified optional deps, seeded smoke path, downloaded instance files, and BKS metadata | IN PROGRESS. Starter dataset download/import is done, BKS-backed hierarchical gap handling is active, and a bounded OR-Tools/PyVRP slice has been run. Next step is broader multi-family/multi-seed execution across all selected A/B/C/R/RC families before treating rankings as release-grade. |
...
| 8 | Full regression gate before release branch | Cross-phase | Depends on chosen release scope | Run academic/core pytest, focused optimizer API tests, and frontend typecheck together; document known optional skips/failures separately. Current blocker: local `pydantic-core` version mismatch prevents optimizer API collection until dependency versions are aligned. |
...
Recommended next task: broaden real CVRPLIB/Solomon validation with multi-seed runs and solver time-limit variants; treat pyvroom's Windows matrix buffer compatibility as a separate optional task because VROOM CVRPTW rows are already excluded safely when native execution fails.
