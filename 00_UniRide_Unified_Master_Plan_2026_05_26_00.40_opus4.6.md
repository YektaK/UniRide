...
|:------|:-----|:------|:-----------|:------|
| 1 | Broaden CVRPLIB/Solomon benchmark execution | 3.2 / 3.3 | Depends on verified optional deps, seeded smoke path, downloaded instance files, and BKS metadata | IN PROGRESS. Starter dataset download/import is done, BKS-backed hierarchical gap handling is active, and bounded OR-Tools/PyVRP slices have run across selected A/B/C/R/RC families. Next step is multi-seed execution across all selected families before treating rankings as release-grade. |
| 2 | Holistic solver constraint-validation pass | 3.3 / 4.2 | Mostly done; remaining work is broader dataset execution and VROOM Windows matrix compatibility | Root causes identified and fixed/guarded. Scalar CVRP demand, Solomon distance semantics, CVRPTW windows/service times, defensive returned-route validation, and hierarchical VRPTW gap reporting are implemented. Remaining work: optional pyvroom Windows matrix buffer compatibility plus broader real-dataset execution. |
| 3 | Promotion-quality config generation | 4.1 / 4.2 | Depends on tasks 1 and 2 with real datasets | FUNCTIONAL for current local evidence. Promotion generation includes feasible benchmark rows with empty `{}` params, excludes failed/infeasible rows, prefers non-smoke evidence, normalizes near-zero gaps, and reads benchmark params from result metadata. Continue by broadening real runs before treating promoted configs as release-grade. |
...
| 8 | Full regression gate before release branch | Cross-phase | Depends on chosen release scope | CURRENTLY GREEN for the main local gate: academic/core pytest plus focused optimizer API tests pass, and frontend typecheck passes. Remaining release-scope work is broader optimizer API/frontend runtime coverage if requested. |
...
- Task 49: Repeatable matrix-validation profiles added for academic benchmark readiness. `academic_benchmark.matrix_validation_plan` now supports `--profile quick-real` for the existing curated real-instance validation and `--profile all-imported` for all stored CVRP/CVRPTW rows in SQLite. The all-imported profile defaults to one `time10-scale1000` variant, OR-Tools + PyVRP, `n_runs=1`, and `limit=100000`, matching the recent all-imported validation shape while still allowing explicit CLI overrides for problems, problem types, runs, limit, and variants. Verification: `python -m pytest academic_benchmark\tests\test_matrix_validation_plan.py -q` passes with 5 tests. Broader academic-suite verification was attempted but blocked by current Codex quota enforcement, so it remains a follow-up gate before marking this profile fully release-ready.

Recommended next task: when quota allows, run the broader academic/core verification gate for the new matrix-validation profile, then continue production strategy migration only where `optimizer_api/strategies/*` still owns critical algorithm behavior; current OR-Tools/PyVRP academic benchmark path is stable enough for repeatable local testing.
