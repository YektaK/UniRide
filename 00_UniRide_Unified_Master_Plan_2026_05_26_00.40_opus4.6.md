...
| 2 | Holistic solver constraint-validation pass | 3.3 / 4.2 | Mostly done; remaining work is broader dataset execution and VROOM Windows matrix compatibility | Root causes identified and fixed/guarded. Scalar CVRP demand was being collapsed to unit SW/SO-style demand; CVRPLIB starter rows now match/clear BKS expectations for OR-Tools/PyVRP. Solomon now uses double-precision Euclidean distances. OR-Tools, PyVRP, and VROOM receive CVRPTW time-window/service-time inputs; returned PyVRP/VROOM routes are defensively validated, and VROOM no longer falls back to non-TW sweep for CVRPTW. Remaining work: broader real dataset execution and optional pyvroom Windows buffer compatibility. |
...
| 4 | Promotion-quality config generation | 4.1 / 4.2 | Depends on tasks 1 and 2 with real datasets | DONE for current local evidence. Promotion generation now includes feasible benchmark rows with empty `{}` params when `--allow-empty-params` is used, so deterministic/core/holistic CVRP and CVRPTW rows are no longer silently excluded. Continue by broadening real runs before treating configs as release-grade. |
