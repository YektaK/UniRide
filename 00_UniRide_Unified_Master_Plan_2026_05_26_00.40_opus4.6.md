...
- Task 55: Split production wrapper dead helper cleanup completed. `GASplitStrategy`, `GAEnhancedSplitStrategy`, `PSOSplitStrategy`, `GWOSplitStrategy`, and `HHOSplitStrategy` no longer own unused private operator/evaluation/local-search helpers; those behaviors remain in `uniride_core` split engines and the production wrappers keep request/matrix/demand mapping plus direct `solve_*_split()` delegation. Stale operator imports and unused best-solution state fields were removed. Verification: RED guards failed before cleanup as expected; after cleanup `python -m pytest optimizer_api\tests\test_strategy_no_legacy_algorithm_helpers.py optimizer_api\tests\test_split_strategy_core_delegation.py optimizer_api\tests\test_all_strategies_smoke.py -q` passes with 11 tests, and focused migration suite passes with 13 tests.
- Task 56: Hybrid split base unused helper cleanup completed and regression gate refreshed. `HybridSplitBaseStrategy` no longer owns unused `_nearest_neighbor_tour`; the active shared helper is `_build_distance_matrix()`, which delegates to `uniride_core.adapters.string_matrix_builder.build_string_distance_matrix`. Verification: RED guard failed before cleanup as expected; after cleanup `python -m pytest optimizer_api\tests\test_strategy_no_legacy_algorithm_helpers.py optimizer_api\tests\test_split_strategy_core_delegation.py -q` passes with 11 tests. Broader chunked gate is green: `academic_benchmark\tests` 194 passed, `uniride_core\tests` 204 passed, focused optimizer compatibility/migration subset 52 passed, and `npm run typecheck` passed.
- Task 57: Core/API architecture boundary guard added. `uniride_core/tests/test_architecture_boundaries.py` now scans `uniride_core` Python imports through AST and fails if any core module imports `optimizer_api`, making the core-first dependency rule explicit and regression-tested. Edge strategy helper audit found `base_strategy.py`, SOTA config/response helpers, holistic response helpers, and promoted-config loader are mapping/config helpers over core or academic contracts rather than solver owners. Verification: `python -m pytest uniride_core\tests\test_architecture_boundaries.py -q` passes; `python -m pytest uniride_core\tests -q` passes with 205 tests.
...
- Task 53: Active Numba benchmark helper imports moved to the core algorithm package. `.temp_master_numba.py`, `optimizer_api/tests/run_interactive_benchmark_v2_numba.py`, and `optimizer_api/tests/run_smart_benchmark_numba.py` now import `LocalSearchType` / Numba local-search logic from `uniride_core.algorithms.local_search_numba` instead of `optimizer_api.utils.local_search_numba`; the smart benchmark algorithm hash target also points at the core source file. `uniride_core/tests/test_architecture_boundaries.py` now guards active benchmark helpers against importing algorithm shims from `optimizer_api.utils`, while still allowing explicit compatibility tests and archived docs to reference those shims. Verification: the boundary test passes, both touched UTF-8 benchmark scripts compile, and the tracked UTF-16 temp helper parses through AST.
- Task 54: Route scheduling helper moved to `uniride_core`. `uniride_core.algorithms.route_scheduling` now owns `calculate_scheduled_times()` and `minutes_to_time()` without importing API schemas; it uses duck-typed route/request fields so `optimizer_api` can pass existing Pydantic models safely. `optimizer_api.utils.scheduling` is now a compatibility re-export only. Verification: focused core/API scheduling tests pass with 6 tests.
- Task 55: Time-window extraction moved to `uniride_core`. `uniride_core.algorithms.time_window_extractor` now owns `TimeWindow`, `TimeWindowExtractor`, and `weekly_schedule_to_optimization_input()` while `optimizer_api.utils.time_window_extractor` remains a compatibility re-export. Migration also fixed the copied `TimeWindow.__post_init__` edge case so inverted out-of-range bounds are ordered before both values are clamped to `[0, 1440]`. Verification: focused core/API extractor tests pass with 10 tests; full `uniride_core\tests` passes with 212 tests.
- Task 56: IE resource profiling moved to `uniride_core`. `uniride_core.algorithms.resource_profiler` now owns `ResourceProfiler`, `ResourceBlock`, `HourlyDemand`, `Bottleneck`, `TimeShiftSuggestion`, and the time conversion helpers using structural student/vehicle objects instead of importing API schemas. `optimizer_api.utils.resource_profiler` is now a compatibility re-export used by existing routes/tests. Verification: focused core/API resource-profiler tests pass with 23 tests; full `uniride_core\tests` passes with 215 tests.
- Task 57: Time-window violation tracking moved to `uniride_core`. `uniride_core.algorithms.time_window_violation_tracker` now owns `ViolationType`, `TimeWindowViolation`, `ViolationReport`, `TimeWindowViolationTracker`, and `create_tracker_from_students()`; `optimizer_api.utils.time_window_violation_tracker` is a compatibility re-export. Verification: focused core/API violation-tracker tests pass with 3 tests; full `uniride_core\tests` passes with 217 tests.
- Task 58: DataLoader boundary pinned as API/application glue. `optimizer_api.utils.data_loader` remains outside core because it owns Supabase loading, environment-driven cache refresh, and request-time matrix access, while its matrix fallback helpers already delegate to canonical core distance functions. New regression coverage prevents DataLoader from importing solver engines, metaheuristic modules, production strategies, or other route-search owners. Verification: focused DataLoader boundary/delegation tests pass with 4 tests.
- Task 59: Resource profiler demo import repaired after the core migration. `optimizer_api/utils/resource_profiler_demo.py` now imports only `ResourceProfiler` from the compatibility shim and imports API schema objects (`StudentNode`, `VehicleConfig`) from `models.schemas`, keeping the shim focused on core-owned profiling helpers. Verification: demo script compiles and focused resource-profiler tests pass with 23 tests.
- Task 60: Core utility shim identity coverage added. `optimizer_api/tests/test_core_utility_shims.py` now verifies scheduling, time-window extraction, and resource-profiling API utility modules re-export the exact core-owned objects by identity. Verification: focused utility-shim regression suite passes with 42 tests.
- Task 61: Archive/proposed-change boundary guard added. `uniride_core/tests/test_archive_boundaries.py` now scans active Python roots (`academic_benchmark`, `optimizer_api`, `src`, `uniride_core`) and fails if active code imports `_archived` modules, `.proposed_changes`, `docs.old`, or mutates `sys.path` toward archived/proposed directories. This allows retained historical code to remain in the repository without becoming an accidental execution path. Verification: focused archive-boundary tests pass with 2 tests; full `uniride_core\tests` passes with 219 tests.
- Task 62: Academic CLI entrypoint coverage added. `academic_benchmark/tests/test_cli_entrypoints.py` now verifies `run_matrix_benchmark.main()` parses CSV algorithm/problem/problem-type args, JSON global/per-algorithm params, seed/run/limit controls, and smoke seeding flags without executing heavy solver work; it also verifies `promotion_manager.main()` dry-run output and validation-failure exit behavior. Verification: focused CLI tests pass with 4 tests; full `academic_benchmark\tests` passes with 198 tests.
- Task 63: Frontend academic read-only service coverage completed. `src/services/benchmark-service.test.ts` now covers `fetchAcademicLeaderboard()` and `fetchAcademicBestResult()` URL encoding, happy-path JSON passthrough, and status-error behavior, complementing existing academic problem, matrix-native run, status, and results tests. Verification: focused service tests pass with 10 tests; `npm run typecheck` passes.
- Task 64: Backend academic read-only endpoint coverage completed. `optimizer_api/tests/test_benchmark_router_problem_loading.py` now covers academic leaderboard forwarding/default handling and academic best-result success/404 behavior against the `academic_benchmark.results_reader` boundary. Verification: focused router tests pass with 17 tests; benchmark router/schema/param-forwarding tests pass with 28 tests.
- Task 65: Matrix-native web benchmark request mapping fixed and pinned. `src/app/api/benchmark/run/route.ts` now builds Python benchmark requests through `src/lib/benchmark-backend-request.ts`, preserving frontend snake_case settings (`n_runs`, `skip_cached`) and forwarding `matrix_native` / `academic_matrix` execution modes instead of silently defaulting to three production-style runs. Verification: mapper/service Vitest tests pass with 12 tests; `npm run typecheck` passes; backend benchmark router/schema/param-forwarding tests pass with 28 tests.
- Task 66: HHO TSP production wrapper migration guard added. `optimizer_api/tests/test_strategy_no_legacy_algorithm_helpers.py` now explicitly prevents legacy HHO operator helpers from returning to `HarrisHawksOptimizerStrategy`, keeping active HHO TSP logic in `uniride_core.algorithms.tsp_meta_engines.solve_hho_tsp`. Verification: production wrapper guard/RNG/all-strategy smoke subset passes with 15 tests.
- Task 67: Broad optimizer API regression suite repaired. `optimizer_api.models.schemas` now preserves legacy CVRPTW schema helpers (`TimeWindow.from_time_string()`, `TimeWindow.to_time_string()`, `StudentNode.get_time_window()`, and weekly schedule window helpers), validates HH:MM values, keeps optional per-student direction compatibility, and allows matrix-backed requests without inline coordinates while still validating coordinate bounds when coordinates are provided. Verification: `optimizer_api\tests\test_cvrptw_phase1.py` passes with 27 tests; full `optimizer_api\tests` passes with 236 tests.
- Task 68: Frontend production build gate fixed. Next 16's default Turbopack build panicked on the Windows workspace path containing `ü`; `package.json` now uses the supported Webpack build path (`next build --webpack`) and `next.config.ts` pins both `outputFileTracingRoot` and `turbopack.root` to the repo root to avoid parent-lockfile root inference. Verification: `npm run build` passes and renders all 57 app routes.
- Task 69: Holistic production wrapper delegation coverage completed. `optimizer_api/tests/test_holistic_strategy_core_delegation.py` now proves OR-Tools, PyVRP, and PyVRP-Alt production wrappers pass matrix, coordinates, disability types, capacities, route duration, vehicle count, and time-limit settings into the wrapper-imported `uniride_core` solver functions while keeping response construction in `optimizer_api`. Verification: focused holistic delegation/response tests pass with 7 tests; production wrapper migration subset passes with 22 tests.
- Task 70: Core reverse-dependency guard strengthened. `uniride_core/tests/test_architecture_boundaries.py` now enforces the full documented boundary invariant: `uniride_core` must not import either `optimizer_api` or `academic_benchmark`. Verification: focused architecture/archive boundary tests pass with 4 tests; full `uniride_core\tests` passes with 219 tests.
- Task 71: Active academic benchmark application-boundary cleanup completed. `academic_benchmark/bildiri2026/data_manager.py` no longer reads TSPLIB files from `optimizer_api/tests`; it uses academic-owned local data or an explicit caller-provided source directory. `academic_benchmark/tests/test_benchmark_robustness.py` now checks academic-owned worker modules instead of optimizer-api test helpers, and `academic_benchmark/tests/test_bildiri_data_manager.py` guards active academic Python modules against application-layer references while excluding archived historical scripts. Verification: focused data-manager/robustness tests pass with 6 tests; full `academic_benchmark\tests` passes with 202 tests.
- Task 72: Release-scope local verification refreshed after strategy-boundary audit. Remaining active production wrappers were audited against existing core-delegation guards; no new algorithm-critical ownership was found in `optimizer_api/strategies/*`. Verification: focused strategy-boundary suite passes with 18 tests, full `optimizer_api\tests` passes with 239 tests, full `uniride_core\tests` passes with 219 tests, `npm run build` passes with Webpack and renders all 57 app routes, and sequential `npm run typecheck` passes. Note: running `npm run build` and `npm run typecheck` concurrently can produce transient missing `.next/types` errors because the build rewrites generated Next types.
- Task 73: Benchmark web runtime smoke and Next 16 proxy hardening completed. The admin benchmark page now checks backend availability through the local Next proxy (`/api/benchmark/health`) instead of a hard-coded malformed strategy URL. `checkBenchmarkApiHealth()` is covered by service tests for OK and fetch-failure paths. The former global `middleware.ts` / `next-intl` interception caused all app pages to return 404 under Next 16 despite successful builds; it has been replaced by a scoped `src/proxy.ts` that only protects `/api/admin/:path*`, preserving admin API bearer-token enforcement while allowing app routes and benchmark APIs to resolve normally. Runtime verification against `next start -p 9003`: `/`, `/admin/benchmark`, and `/dashboard` return 200, `/api/benchmark/health` returns 200, and unauthenticated `/api/admin/users` returns 401. Matrix-native web execution through the Next proxy completed with run id `web-runtime-smoke-20260605-proxy`, one persisted result, objective `4`, and editable params preserved in metadata (`{"max_iterations":5}`). Verification: `npm run test -- src/services/benchmark-service.test.ts --run` passes with 11 tests, `npm run typecheck` passes, and `npm run build` passes with Webpack and no Next 16 middleware deprecation warning.
- Task 74: Bounded quick-real academic validation refreshed. `python -m academic_benchmark.matrix_validation_plan --profile quick-real --run-id-prefix matrix-validation-quick-real-20260605 --variants time5-scale1000:5` completed as run id `matrix-validation-quick-real-20260605-time5-scale1000` with 60 saved rows and `errors=[]`. Summary: 60 feasible rows, 0 failed rows, 0 capacity-violation rows, 0 time-window-violation rows, 24 CVRP rows, 36 CVRPTW rows, 21 positive vehicle-gap rows, 21 distance-gap-suppressed rows, and 0 unexpected distance gaps when vehicle count is worse than BKS. This is current academic DB evidence for OR-Tools/PyVRP on the curated quick-real CVRPLIB/Solomon slice; broader all-imported and multi-seed profiles remain the release-grade evidence path when promoting configs.
- Task 75: Promotion dry-run sanity refreshed from current feasible SQLite evidence. `python -m academic_benchmark.promotion_manager --dry-run --allow-empty-params --min-evidence-runs 2 --min-configs 1 --limit 100000` completed without writing files and reported `configs=48`. This confirms the promotion builder can still derive config candidates from feasible academic rows while enforcing the multi-row evidence gate.

Recommended next task: run broader live academic benchmark evidence only if promoting configs or FCM-SRS beyond monitored status; otherwise move to UI/runtime smoke testing of the benchmark pages against the local API.
