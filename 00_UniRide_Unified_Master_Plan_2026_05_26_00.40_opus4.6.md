...
Recommended next task: rerun broader CVRPLIB/Solomon matrix benchmarks with feasible-row filtering (`execution_failed = false`, `capacity_violations = 0`, `tw_violations = 0`) before promotion-quality leaderboard/config generation; optionally investigate pyvroom's Windows matrix buffer compatibility separately.
