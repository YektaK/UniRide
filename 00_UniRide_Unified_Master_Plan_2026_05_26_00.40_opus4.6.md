...
|:------|:-----|:------|:-----------|:------|
| 1 | Broad CVRPLIB/Solomon import and benchmark execution | 3.2 / 3.3 | Depends on verified optional deps, seeded smoke path, and local/downloaded instance files | Import CLI, non-mutating scan CLI, and generic matrix benchmark CLI are available. Active workspace currently has no real `.vrp`/Solomon dataset files, so broad import is waiting on downloaded/supplied instance files. Smoke CVRP/CVRPTW execution is complete. |
| 2 | Holistic solver comparison pass | 3.3 / 4.2 | Smoke pass complete; real comparison depends on broad dataset import | OR-Tools, PyVRP, and VROOM smoke comparison is persisted. Repeat on real CVRPLIB/Solomon families after import; record infeasible/unsupported cases explicitly rather than hiding them. |
...
Recommended next task: download or provide real CVRPLIB `.vrp` and Solomon CVRPTW instance files, inspect them with `python -m academic_benchmark.cvrplib_manager scan <paths>`, import them with `python -m academic_benchmark.cvrplib_manager import <paths>`, then rerun `python -m academic_benchmark.run_matrix_benchmark --problem-types cvrp cvrptw ...` for broad evidence.
