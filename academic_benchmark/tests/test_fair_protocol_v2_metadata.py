...
def test_v1_config_remains_loadable(tmp_path: Path):
    ...
    assert set(loaded.algorithms) == set(ALGORITHMS)
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda blocks: blocks["Core-TwoOpt-TSP"].pop("first_improvement"),
        lambda blocks: blocks["Core-TwoOpt-TSP"].__setitem__("first_improvement", 1),
        lambda blocks: blocks["Core-ThreeOpt-TSP"].pop("first_improvement"),
        lambda blocks: blocks["Core-ThreeOpt-TSP"].__setitem__("window", 1),
        lambda blocks: blocks["Core-ThreeOpt-TSP"].__setitem__("window", True),
...
def test_first_improvement_metadata_is_explicit_and_directed_cost_is_preserved():
    ...
    result = _run("Core-ThreeOpt-TSP", directed=True, first_improvement=True)
...
def test_local_search_termination_reasons_distinguish_budget_iteration_and_local_optimum():
    exhausted = _run("Core-TwoOpt-TSP", budget=1, max_iterations=20)
    capped = _run("Core-TwoOpt-TSP", budget=100, max_iterations=0)
    local_optimum = _run("Core-TwoOpt-TSP", budget=100, max_iterations=20)
...
def test_v2_metadata_serializes_and_survives_fair_pilot_record(tmp_path: Path):
    ...
    problem = _problem(directed=True)
    algorithm_id = "Core-ThreeOpt-TSP"
    ...
    result, decision, elapsed_ms = _execute_preflighted_run(
        ...
        problem=problem,
        params={**config.algorithms[algorithm_id], "fair_comparison": config.fair_comparison},
        seed=seed,
        run_idx=0,
        ...
        runtime_backends=RuntimeBackendAvailability(True, False, "Python test fixture"),
