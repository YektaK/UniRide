#!/usr/bin/env python3
"""
Aşama 3: Dirençli Toplu Benchmark — YAEM 2026
"""
import os
import sys
import json
...
FACTORIES = {
    ...
    "GA": GAOptimizer,
    "PSO": PSOOptimizer,
    "GWO-2opt": GWOOptimizer,
    "GWO-Pure": PureGWOOptimizer,
    "GWO-LKH": GWO_LKH_Optimizer,
    "GWO-ALNS": GWO_ALNS_Optimizer,
    "HHO-2opt": HHOOptimizer,
    "HHO-Pure": PureHHOOptimizer,
    "HHO-LKH": HHO_LKH_Optimizer,
    "HHO-ALNS": HHO_ALNS_Optimizer,
}
...
def main():
    print("=" * 70)
    print("Aşama 3: YAEM 2026 — Dirençli Toplu Benchmark")
    print("=" * 70)
    
    DB_PATH = os.path.join(SCRIPT_DIR, "data", "universal_parameters_db.json")
    if not os.path.exists(DB_PATH):
        print(f"[HATA] {DB_PATH} bulunamadı. Lütfen önce 'extract_universal_params.py' çalıştırın.")
        return
        
    with open(DB_PATH, "r", encoding="utf-8") as f:
        db = json.load(f)
        
    print("\n--- Kayıtlı Optimizasyon Modelleri ---")
    for entry in db:
        algo = entry["algorithm"]
        prob = entry["problem"]
        score = entry["best_mean_length"]
        if isinstance(score, (int, float)):
            print(f"  [{entry['id']}] {algo:<12} (Eğitim: {prob:<8}) -> Skor: {score:.2f}")
        else:
            print(f"  [{entry['id']}] {algo:<12} (Eğitim: {prob:<8}) -> Skor: {score}")
