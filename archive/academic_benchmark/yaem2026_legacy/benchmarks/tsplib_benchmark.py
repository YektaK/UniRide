"""
TSPLIB Benchmark Runner for YAEM 2026

Runs GA, PSO, GWO, HHO, 2-opt, 3-opt, Or-opt on TSPLIB problems.
"""
...
from core import (
    ...
    GWOOptimizer, HHOOptimizer,
    PureGWOOptimizer, PureHHOOptimizer,
    TSPResult
)
...
def run_benchmark_suite(
    ...
    algorithms = {
        "2-opt": lambda seed: TwoOptSolver(
            max_iterations=5000, first_improvement=False,
            multi_start=True, num_starts=20, random_seed=seed
        ),
        "3-opt": lambda seed: ThreeOptSolver(
            max_iterations=1000, first_improvement=False,
            multi_start=False, random_seed=seed
        ),
        "Or-opt": lambda seed: OrOptSolver(
            max_iterations=1000, max_segment_size=3,
            multi_start=False, random_seed=seed
        ),
        "GA": lambda seed: GAOptimizer(
            ...
            max_no_improvement=80, random_seed=seed
        ),
        "PSO": lambda seed: PSOOptimizer(
            swarm_size=50, max_iterations=300,
            inertia_weight=0.729, cognitive_coeff=1.49445,
            social_coeff=1.49445, max_no_improvement=80,
            random_seed=seed
        ),
        "GWO": lambda seed: GWOOptimizer(random_seed=seed),
        "GWO-Pure": lambda seed: PureGWOOptimizer(random_seed=seed),
        "HHO": lambda seed: HHOOptimizer(random_seed=seed),
        "HHO-Pure": lambda seed: PureHHOOptimizer(random_seed=seed),
    }
    ...
    for prob_name in problem_names:
        ...
        for algo_name, algo_factory in algorithms.items():
            ...
            print(f"      Mean time:       {sum(times)/len(times):.2f} ms")
            if gaps:
    ...
    print(f"{'='*70}")
    return {"timestamp": timestamp, "results": results, "json_path": json_path, "csv_path": csv_path}
...
def main():
    ...
    parser.add_argument("--problems", nargs="+", default=["eil51", "berlin52", "st70", "eil76", "eil101"], help="TSPLIB problem names")
    ...
    run_benchmark_suite(
        problem_names=args.problems, num_runs=args.runs,
        data_dir=args.data_dir, output_dir=args.output,
