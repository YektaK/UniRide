...
The benchmark and CLI endpoints now require `X-Internal-API-Key` via the router-level `require_tenant_authorization` dependency (tenant keys or the shared internal ops key). The archived CLI path-traversal vulnerability (caller-selected `filepath`) was replaced with `_resolve_cli_filename()`, which restricts filenames to a configured search directory. Per-tenant authorization and rate limiting are implemented.
