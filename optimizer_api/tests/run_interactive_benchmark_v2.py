...
# -*- coding: utf-8 -*-
"""
Interactive TSPLIB Benchmark Runner with Real Coordinates
...
Ozellikler:
- TSPLIB .tsp dosyalarini otomatik indirir ve okur
- Gercek koordinatlar ile calisir
...
Gerekli: pip install requests (dosya indirme icin)
"""
...
def parse_tsplib_file(filepath: str) -> Optional[Dict]:
    ...
    Returns:
        Dictionary with name, dimension, coordinates
    """
    try:
...
def load_tsplib_problem(problem_name: str, optimal: int, category: str) -> Optional[TSPLIBProblem]:
    ...
    Args:
        problem_name: Name of the problem (e.g., 'berlin52')
        optimal: Known optimal solution value
        category: Problem category (small, medium, large)
    
    Returns:
        TSPLIBProblem instance or None if failed
    """
...
def calculate_tour_length(tour: List[int], coordinates: List[Tuple[float, float]]) -> int:
    ...
    for i in range(len(tour) - 1):
        p1 = coordinates[tour[i] - 1]
        p2 = coordinates[tour[i + 1] - 1]
        total += tsplib_distance(p1, p2)
    
    # Close tour
    p1 = coordinates[tour[-1] - 1]
    p2 = coordinates[tour[0] - 1]
    total += tsplib_distance(p1, p2)
...
def run_interactive_benchmark():
    ...
    print("\n" + "="*100)
    print("INTERAKTIF TSPLIB BENCHMARK (Gercek Koordinatlar)")
    print("="*100)
    ...
    # Load problems for selected categories
    problems_to_run = []
